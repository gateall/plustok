...
<div class="card" style="margin-top:16px;border:1px solid #e5e7eb">
  ...
  <div style="text-align:right;display:flex;justify-content:flex-end;gap:8px">
    ...
    <?php if (empty($c['email'])): ?>
      <span class="muted" style="font-size:12px;align-self:center">고객 이메일이 없어 발송할 수 없습니다</span>
    <?php else: ?>
      <button type="button" id="btn-send-reply" class="btn" style="padding:4px 12px;font-size:12px;background:#059669;color:#fff" disabled>📧 고객에게 이메일 발송</button>
    <?php endif; ?>
  </div>
</div>
...
(function() {
  ...
  if (replyText) {
    replyText.addEventListener('input', function() {
      const has = replyText.value.trim().length > 0;
      if (btnCopy) btnCopy.disabled = !has;
      if (btnSend) btnSend.disabled = !has;
    });
  }
  ...
  if (btnSend) {
    btnSend.addEventListener('click', async function() {
      ...
      if (!text) return;
      if (!confirm('이 내용을 고객 이메일로 발송할까요?')) return;
      btnSend.disabled = true;
      const origText = btnSend.innerText;
      btnSend.innerText = '⏳ 발송 중...';
      try {
        const formData = new URLSearchParams();
        formData.append('_csrf', csrfToken);
        formData.append('consult_id', consultId);
        ...
        const res = await fetch('/admin/consults/send_reply.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: formData
        });
