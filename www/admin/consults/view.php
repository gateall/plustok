...
<div class="consult-view-page">
<div class="consult-view-header">
  <div class="consult-view-header__main">
    <a href="/admin/consults/" class="consult-view-back">← 목록</a>
    <h1 class="consult-view-title">
      <?= e($c['consult_no']) ?>
      <span class="badge st-<?= e($c['status']) ?>"><?= e(CONSULT_STATUSES[$c['status']] ?? $c['status']) ?></span>
    </h1>
  </div>
  <?php if (can_manage()): ?>
    <form method="post" class="consult-view-delete-form" onsubmit="return confirm('이 상담을 완전히 삭제할까요? (되돌릴 수 없음)')">
      <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
      <input type="hidden" name="action" value="delete">
      <button type="submit" class="btn danger consult-view-delete-btn">상담 삭제</button>
    </form>
  <?php endif; ?>
</div>
...
<!-- 레이아웃 컨테이너: PC 2단 / 모바일 1단 (order로 재배치) -->
<div class="consult-view-main">
  <div class="consult-view-col consult-view-col--primary">
    <!-- 상담정보 -->
    <section class="cv-section cv-info card">
      ...
      <?php if ($detail): ?>
        ...
        <table class="consult-detail-table">
          <tbody>
            <?php foreach ($detail as $k => $v): ?>
              <tr>
                <th><?= e((string)$k) ?></th>
                <td><?= e(is_scalar($v) ? (string)$v : json_encode($v, JSON_UNESCAPED_UNICODE)) ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
    ...
    <!-- AI 요약 -->
    <section class="cv-section cv-ai-summary card card--ai-summary">
      <div class="cv-ai-summary-head">
        <h3>✨ AI 상담 요약</h3>
        <div class="cv-ai-summary-actions">
          <span id="ai-summary-time" class="muted cv-ai-summary-time"><?= $c['ai_summary_at'] ? e($c['ai_summary_at']) . ' 생성' : '' ?></span>
          <button type="button" id="btn-ai-summary" class="btn btn--compact">
            <?= $c['ai_summary'] ? '🔄 재생성' : '✨ AI 요약 생성' ?>
          </button>
        </div>
      </div>
      <div id="ai-summary-text" class="cv-ai-summary-body">
    ...
    <!-- 실시간 메시지 -->
    <?php if ($roomId): ?>
    <section class="cv-section cv-chat card consult-chat-card" id="consult-messaging" data-room-id="<?= e($roomId) ?>">
  ...
  <div class="consult-view-col consult-view-col--secondary">
    <!-- 진행상태 -->
    <section class="cv-section cv-status card card--status">
      <h3 class="card--status-title">진행 상태 변경</h3>
      <form method="post" class="cv-status-form">
        <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
        <input type="hidden" name="action" value="status">
        <div class="cv-form-row cv-form-row--2col">
          <select name="status" class="cv-input">
            <?php foreach (CONSULT_STATUSES as $k => $v): ?>
              <option value="<?= e($k) ?>" <?= $c['status'] === $k ? 'selected' : '' ?>><?= e($v) ?></option>
            <?php endforeach; ?>
          </select>
          <button type="submit" class="btn cv-btn-status">변경</button>
        </div>
        <div class="cv-form-field">
          <label class="cv-label">변경 메모 (선택)</label>
          <input type="text" name="note" maxlength="255" class="cv-input">
        </div>
      </form>

      <h3 class="card--status-title card--status-title--sub">담당자 배정</h3>
      <form method="post" class="cv-assign-form">
        <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
        <input type="hidden" name="action" value="assign">
        <div class="cv-form-row cv-form-row--2col">
          <select name="manager_id" class="cv-input">
          ...
          <button type="submit" class="btn cv-btn-assign">배정</button>
        </div>
      </form>
    </section>

    <!-- 고객정보 -->
    <section class="cv-section cv-customer card">
      <h3 style="margin-top:0">고객 및 상담 정보</h3>
      <div class="table-responsive">
      <table class="consult-info-table"><tbody>
    ...
    <!-- AI 인사이트 -->
    <section class="cv-section cv-ai-insight card card--ai-insight">
      <div class="cv-ai-insight-head">
        <h3>🤖 AI 인사이트</h3>
        <button type="button" id="btn-ai-analyze" class="btn btn--compact">
          <?= $c['ai_analyzed_at'] ? '재분석' : '분석실행' ?>
        </button>
      </div>
      
      <div class="cv-ai-insight-grid">
      ...
      <div class="cv-ai-tags" id="ai-val-tags">
        <?= e($c['tags'] ?? '') ?>
      </div>
    </section>
  </div>

  <!-- 하단 탭 영역 (메모, 첨부파일) — mobile stack order 7 -->
  <section class="cv-section cv-tabs card consult-view-tabs">
  <!-- 탭 네비게이션 -->
  <div class="consult-tab-nav" role="tablist">
    <button type="button" class="tab-btn active" data-tab="tab-reply" role="tab">✉️ 고객 답변 발송</button>
    <button type="button" class="tab-btn" data-tab="tab-memo" role="tab">📝 내부 메모</button>
    <button type="button" class="tab-btn" data-tab="tab-file" role="tab">📎 첨부 파일</button>
    <button type="button" class="tab-btn" data-tab="tab-history" role="tab">🕒 상태 이력</button>
  </div>
  ...
  <div id="tab-reply" class="tab-content" style="display:block;">
    ...
    <div class="cv-reply-actions">
      <div class="cv-reply-channels">
        <label><input type="checkbox" id="reply-chk-email" checked> 이메일 발송</label>
        <label><input type="checkbox" id="reply-chk-sms"> 문자 발송</label>
        <label><input type="checkbox" id="reply-chk-kakao"> 카카오 알림톡</label>
        <label class="cv-reply-close"><input type="checkbox" id="reply-chk-close"> 상담 종료</label>
      </div>
      <div class="cv-reply-btns">
        <button type="button" id="btn-reply-save" class="btn btn--secondary">임시저장</button>
        <button type="button" id="btn-reply-send" class="btn btn--primary">고객에게 발송</button>
      </div>
    </div>
