...
export default function ConsultSummaryCards({ stats }: ConsultSummaryCardsProps) {
  return (
    <div
      className="consult-summary-cards grid grid-cols-2 gap-2 min-[769px]:grid-cols-3 min-[769px]:grid-cols-6 min-[769px]:gap-3"
      aria-label="상담 현황 요약"
    >
      {CARDS.map(({ key, label }) => (
        <div
          key={key}
          className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 min-[769px]:px-4 min-[769px]:py-3"
        >
          <p className="text-xs font-medium text-slate-500">{label}</p>
          <p className="mt-0.5 text-xl font-bold tabular-nums text-slate-900 min-[769px]:text-2xl">
            {stats[key]}
          </p>
        </div>
      ))}
